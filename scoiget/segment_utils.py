...
def split_data_by_cluster(data, pred_label):
    ...
    Args:
        data (np.ndarray): Array of shape (n_samples, n_features) containing the data to be split.
        pred_label (np.ndarray): Array of cluster labels for each sample.
    
    Returns:
        dict: A dictionary where keys are unique cluster labels and values are corresponding data subsets.
    """
...
def pelt_multi(signal, penalty):
    """
    Perform segmentation using the PELT (Pruned Exact Linear Time) algorithm.

    Args:
        signal (np.ndarray or torch.Tensor): Input signal to segment, with shape (n_samples, n_features).
        penalty (float): Penalty term for adding a new segment.
    
    Returns:
        torch.Tensor: Indices of detected breakpoints.
    """
    n, m = signal.shape
...
def perform_segmentation(clone_cn, chrom_list, eta=1, use_gpu=True):
    """
    Perform segmentation on copy number data using the PELT algorithm.

    Args:
        clone_cn (np.ndarray or torch.Tensor): Input copy number data of shape (n_cells, n_bins).
        chrom_list (list): List of chromosome boundary bins.
        eta (float): Coefficient for penalty calculation.
        use_gpu (bool): Whether to use GPU for computation.
    
    Returns:
        tuple: Tuple containing segmented copy number profile, detected breakpoints, and segmented array.
    """
...
def plot_heatmap_and_segmentation(cluster_dict, chrom_list, output_dir, bin_size, library_id, segmentation_results, method):
    """
    Plot heatmap and segmentation results for all clusters, and save the results.

    Args:
        cluster_dict (dict): Dictionary containing data for each cluster.
        chrom_list (list): List of chromosome boundary bins.
        output_dir (str): Directory to save the images.
        bin_size (int): Bin size used for the data.
        library_id (str): Library ID for naming the files.
        segmentation_results (dict): Segmentation results for each cluster.
...
def plot_segmentation(clone_cn, segment_cn, bp_arr, chrom_list, save_path):
    """
    Plot and save segmentation results for a specific cluster.
    ...
    """
    # Plot segmentation result with original signal
    plt.figure(figsize=(20, 5))
    plt.plot(clone_cn[0], label='Original Signal', color="skyblue")
    for bkp in bp_arr:
    ...
    # Plot segmentation result without original signal
    plt.figure(figsize=(20, 5))
    for bkp in bp_arr:
        plt.axvline(x=bkp, color='red', linestyle='--')
    plt.plot(segment_cn, label='Segmented Signal', linestyle='-', color="darkorange")
